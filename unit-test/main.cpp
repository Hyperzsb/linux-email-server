...
TEST_CASE("SignIn") {
    ...
    printf("!%d\n", sign_in_status);
    REQUIRE(sign_in_status == SIGN_IN_SUCCESS);
    delete feedback;
    feedback = mysql_dao->SignIn("127.0.0.1", "sign_in@catch.org", "password");
    sign_in_status = feedback->status;
    printf("!%d\n", sign_in_status);
    REQUIRE(sign_in_status == SIGN_IN_INVALID_PASSWD);
    delete feedback;
    feedback = mysql_dao->SignIn("127.0.0.1", "sign_in_wrong@catch.org", "passwd");
    sign_in_status = feedback->status;
    printf("!%d\n", sign_in_status);
    REQUIRE(sign_in_status == SIGN_IN_INVALID_ACCOUNT);
    delete feedback;
    delete mysql_dao;
}
